...
def b2str(bS):
    return ''.join(list(map(lambda x: chr(x),bS)))
...
def main():    
    ...
    cypher_list = [c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c_target]
    ...
    #    for c1 in cypher_list:
    #        xorMSG = bytesXor(c,c1)
    #        for j,m in enumerate(xorMSG):
    #            if str(m) in st.ascii_letters and msg_list[i][j] not in st.ascii_letters:
    #                msg_list[i][j] = str(m)
